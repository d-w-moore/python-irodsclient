...
change to `True` in the near future, likely in version v4.0.0 of the library.
...
to an unintended consequence.  It is recommended for all client library users,
in the meantime, that this environment variable should be set True
for all applications that load settings from the environment or from config-files.
...
client application.  If however the invalid setting is attempted by a loading
configuration, this will either be logged or registered as a fatal error,
...
A copy of original value of `IRODS_QUERY_LIMIT` (before overriding values are loaded from the
