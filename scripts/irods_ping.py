...
if __name__ == '__main__':
    ...
    if command == 'iinit':
        ...
        try:
            ...
            with irods.session.iRODSSession(irods_env_file = env_file_path) as sess:
                sess.collections.get('/{0.zone}/home/{0.username}'.format(sess))
        except:
            exit(2)
    else:
        print('\nusage:\n\t{0} connect\nor:'
              '\n\t{0} iinit host myhost port 1247 \\\n'
              '\tuser UUUU zone ZZZZZZZZ password XXXXXX\n'
              ''.format(sys.argv[0]))
        exit(3)
