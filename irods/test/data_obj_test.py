...
from irods.test.helpers import iRODSUserLogins, home_collection
...
class TestDataObjOps(unittest.TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        adm = cls.__sess = helpers.make_session()
    ...
    def test_redirect_in_data_object_put_and_get_with_tickets__issue_452(self):
        for fname,size in {'large':40*MEBI,
                           'small': 1*MEBI }.items():
    ...
    def test_redirect_in_data_object_open__issue_452(self):
        ...
        with self.create_simple_resc(hostname = 'localhost') as rescName:
            try:
                ...
                io.close()
                io = sess.data_objects.open(test_path, 'r')
                self.assertEqual('localhost', io.raw.session.host)
                io.close()
            finally:
