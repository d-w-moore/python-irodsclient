            ...
            try:
                ...
                test_case.assertTrue(
                    wait_till_true(
                        ...
                        and named_irods_data_object(session, logical_path, delete = False).data.size > OBJECT_SIZE // 2,
                    ),
                ...
                test_case.assertTrue(
                    wait_till_true(
                        lambda : int(session.data_objects.get(logical_path).replica_status) < 2
                    )
                )
...
if __name__ == "__main__":
    import getopt
    opts, _ = getopt.getopt('k', sys.argv[1:])
    ...
    object_path = named_irods_data_object(session, delete = True).create().path
    ...
    try:
        # Tell the parent process the name of the data object logical path, the target of the "put".
        print(object_path)
        sys.stdout.flush()
        ...
        if not keep_data_object:
            named_irods_data_object(session, path = object_path, delete = True)
